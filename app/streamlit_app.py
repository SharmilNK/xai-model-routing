...
def get_classification_label(tier):
    """Get a sample classification label based on tier (for demo purposes)."""
    # In a real system, you'd run the actual model inference and get the class name
    # For now, we return example labels based on tier
    labels = {
        'TINY': 'Simple object',
        'LIGHT': 'Common scene',
        'MEDIUM': 'Complex scene',
        'HEAVY': 'Very complex scene'
    }
...
def plot_comparison_radar(f1, f2, n1="Image A", n2="Image B"):
    ...
    fig, ax = plt.subplots(figsize=(4.2, 4.2), subplot_kw=dict(polar=True))  # Reduced from 7x7
    fig.patch.set_facecolor('#000000')
    ax.set_facecolor('#0a0a0a')
    ax.spines['polar'].set_color('#333')
    ax.grid(color='#222', linewidth=0.5)
    
    ax.plot(angles, v1.tolist()+[v1[0]], 'o-', lw=2.0, label=n1, color='#3b82f6', ms=5)
    ax.fill(angles, v1.tolist()+[v1[0]], alpha=0.15, color='#3b82f6')
    ax.plot(angles, v2.tolist()+[v2[0]], 's-', lw=2.0, label=n2, color='#ef4444', ms=5)
    ax.fill(angles, v2.tolist()+[v2[0]], alpha=0.15, color='#ef4444')
    
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(labels, size=8, color='#aaa')
    ax.set_ylim(0, 1)
    ax.set_title('Complexity Comparison', fontsize=12, fontweight='600', color='#fff', pad=15)
    legend = ax.legend(loc='upper right', bbox_to_anchor=(1.18, 1.0), fontsize=8)
    legend.get_frame().set_facecolor('#111')
    for t in legend.get_texts(): t.set_color('#ccc')
    plt.tight_layout()
    return fig
...
def main():
    ...
    # ========================================================================
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.image(img_a, caption="Image A", use_container_width=False, width=400)
            st.markdown(f"""
            <div style="background:{tier_colors[res_a["tier"]]}22;border:2px solid {tier_colors[res_a["tier"]]};
                        padding:0.8rem;border-radius:8px;text-align:center;">
                <h3 style="margin:0;color:{tier_colors[res_a["tier"]]};">{res_a["tier"]}</h3>
                <p style="margin:0;color:#888;">{res_a["measured_latency"]:.1f}ms | {res_a["flops"]}M FLOPs</p>
                <p style="margin:0.5rem 0 0 0;color:#aaa;font-size:0.85rem;">Classified as: {get_classification_label(res_a["tier"])}</p>
            </div>
            """, unsafe_allow_html=True)
        
        with col2:
            st.image(img_b, caption="Image B", use_container_width=False, width=400)
            st.markdown(f"""
            <div style="background:{tier_colors[res_b["tier"]]}22;border:2px solid {tier_colors[res_b["tier"]]};
                        padding:0.8rem;border-radius:8px;text-align:center;">
                <h3 style="margin:0;color:{tier_colors[res_b["tier"]]};">{res_b["tier"]}</h3>
                <p style="margin:0;color:#888;">{res_b["measured_latency"]:.1f}ms | {res_b["flops"]}M FLOPs</p>
                <p style="margin:0.5rem 0 0 0;color:#aaa;font-size:0.85rem;">Classified as: {get_classification_label(res_b["tier"])}</p>
            </div>
            """, unsafe_allow_html=True)
        
        st.markdown("---")
        
        # Comparison Radar (reduced size)
        ...
        # Attention Flow Comparison with expandable view
        st.subheader("Attention Flow Comparison")
        
        # Create figures first
        fig_a = plot_attention_flow(img_a, res_a['activations'])
        fig_b = plot_attention_flow(img_b, res_b['activations'])
        ...
        with st.expander("🔍 Expand for Full-Screen View"):
            st.markdown("#### Image A - Attention Flow")
            st.pyplot(fig_a)
            st.markdown("---")
            st.markdown("#### Image B - Attention Flow")
            st.pyplot(fig_b)
